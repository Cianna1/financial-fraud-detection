...
## 项目概述
本项目是一个基于机器学习的金融欺诈检测系统，集成了XGBoost、MLP神经网络和规则引擎，通过FastAPI提供实时欺诈检测API服务。系统支持Kafka流处理和Prometheus监控，具有高扩展性和可维护性。
...
│   ├── mlp_model.keras         # MLP模型
│   └── scaler.pkl              # 数据标准化器
├── src/                        # 源代码
│   ├── api/                    # API服务
│   │   ├── model_api.py        # FastAPI主服务
│   │   └── main.py             # 应用入口
│   ├── engine/                 # 规则引擎
│   │   └── rules.py            # 欺诈检测规则
│   ├── ml/                     # 机器学习模型
...
python src/ml/dl_mini.py        # 训练MLP模型
```
